...
class Query(QueryBase):
    ...
    def _fix_insert_statement(self, query: str) -> str:
        """Fix INSERT statement formatting."""
        # Match the INSERT statement parts
        match = re.search(
        ...
        formatted_cols = self._fix_column_list(columns)
        formatted_vals = self._fix_values_list(values)
        
        # Reconstruct the INSERT statement
        return f"INSERT INTO {table} ({formatted_cols}) VALUES ({formatted_vals})"

    def _fix_bulk_values(self, query: str) -> str:
        """Fix bulk VALUES clause formatting."""
        # Split on VALUES keyword
        ...
        # Add proper spacing and commas between value sets
    ...
    def _format_sql(self, query: str) -> str:
        ...
        if "INSERT INTO" in query.upper():
            if "VALUES" in query.upper():
                # Single INSERT
                if query.count('VALUES') == 1 and query.count('(') == 2:
                    query = self._fix_insert_statement(query)
                # Bulk INSERT
                else:
                    query = self._fix_bulk_values(query)
